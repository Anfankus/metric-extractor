...
/**
 * this class is borrowed from PMD (pmd.string.util)
 */
public final class CommentUtil {
  ...
  public static String javadocContentAfter(String text, int position) {
    ...
    if (StringUtil.isEmpty(text.substring(position, endPos)) != true) {
    ...
    if (StringUtil.isEmpty(text.substring(endPos, nextEndPos)) != true) {
  ...
  /**
   * Similar to the String.trim() function, this one removes the leading and trailing empty/blank
   * lines from the line list.
   */
  public static List<String> trim(List<String> lines) {
    int firstNonEmpty = 0;
    for (; firstNonEmpty < lines.size(); firstNonEmpty++) {
      if (StringUtil.isEmpty(lines.get(firstNonEmpty)) != true) {
        break;
      }
    }
    ...
    for (; lastNonEmpty > 0; lastNonEmpty--) {
      if (StringUtil.isEmpty(lines.get(lastNonEmpty)) != true) {
        break;
      }
    }
