...
public class JavaSourceCodeChangeClassifier implements SourceCodeChangeClassifier {
  ...
  @Override
  public List<SourceCodeChange> classifySourceCodeChanges(
      List<? extends SourceCodeChange> sourceCodeChanges) {
      ...
      if (insert.getChangedEntity().getUniqueName().endsWith(VOID_RETURN)) {
        ...
        for (Iterator<Delete> it = fDeletes.iterator(); it.hasNext() && check; ) {
          ...
          if ((insert.getRootEntity().getType() == del.getRootEntity().getType())
              ...
              && del.getParentEntity().getUniqueName()
              .equals(insert.getParentEntity().getUniqueName())
              && del.getChangedEntity().getType().isType()
  ...
  private boolean isEqual(SourceCodeEntity entity, EntityType expectedEntityType,
  ...
  private boolean isEqual(StructureEntityVersion entity, EntityType expectedEntityType,
      String expectedEntityName) {
    boolean type = false;
    boolean name = false;
    if (expectedEntityType == null) {
