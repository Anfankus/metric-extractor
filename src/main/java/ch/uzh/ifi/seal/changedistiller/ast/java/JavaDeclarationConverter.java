...
/**
 * Visitor to generate an intermediate tree (general, rooted, labeled, valued tree) out of a field,
...
public class JavaDeclarationConverter extends ASTVisitor {
   ...
   *
   * @param root of the resulting declaration tree
   * @param scanner of the source file that is traversed
   */
  ...
  private void setSourceRange(Node modifiers) {
    SourceCodeEntity firstModifier = ((Node) modifiers.getFirstLeaf()).getSourceCodeEntity();
    SourceCodeEntity lastModifier = ((Node) modifiers.getLastLeaf()).getSourceCodeEntity();
    modifiers.getSourceCodeEntity().setStartPosition(firstModifier.getStartPosition());
    modifiers.getSourceCodeEntity().setEndPosition(lastModifier.getEndPosition());
  }
  ...
  private void visitAbstractMethodDeclaration(AbstractMethodDeclaration methodDeclaration,
    ...
    visitAbstractVariableDeclarations(JavaEntityType.TYPE_PARAMETERS,
  ...
  public boolean visit(ParameterizedSingleTypeReference type, BlockScope scope) {
    ...
    fNodeStack.peek().getSourceCodeEntity().setEndPosition(end);
    return false;
  }
  ...
  private void adjustEndPositionOfParameterizedType(ParameterizedQualifiedTypeReference type) {
    if (hasTypeParameter(type)) {
      ...
      fNodeStack.peek().getSourceCodeEntity()
          .setEndPosition(getLastChildOfCurrentNode().getSourceCodeEntity().getEndPosition() + 1);
    }
  }
  ...
  public boolean visit(SingleTypeReference type, BlockScope scope) {
    pushValuedNode(type,
        prefixWithNameOfParrentIfInMethodDeclaration() + String.valueOf(type.token));
    return false;
  }
  ...
  public boolean visit(ArrayTypeReference arrayType, BlockScope scope) {
    pushValuedNode(arrayType,
        prefixWithNameOfParrentIfInMethodDeclaration() + String.valueOf(arrayType.token));
    return false;
  }
  ...
  public boolean visit(TypeParameter typeParameter, BlockScope scope) {
    push(
        ...
        getSource(typeParameter.sourceStart(),
  ...
  private void visitAbstractVariableDeclarations(JavaEntityType parentLabel,
  ...
  private void adjustSourceRangeOfCurrentNode(int start, int end) {
    fNodeStack.peek().getSourceCodeEntity().setStartPosition(start);
    fNodeStack.peek().getSourceCodeEntity().setEndPosition(end);
  }
  ...
  private void visitList(JavaEntityType parentLabel, ASTNode[] nodes) {
    ...
    if (isNotEmpty(nodes)) {
      ...
      visitList(nodes);
      end = getLastChildOfCurrentNode().getSourceCodeEntity().getEndPosition();
    }
    adjustSourceRangeOfCurrentNode(start, end);
    pop();
  }
  ...
  private void pushValuedNode(ASTNode node, String value) {
    EntityType entityType = fASTHelper.convertNode(node);
    push(entityType, value, node.sourceStart(), node.sourceEnd());
  }

  private void push(EntityType label, String value, int start, int end) {
    Node node = new Node(label, value.trim());
    node.setSourceCodeEntity(
        new SourceCodeEntity(value.trim(), label, new SourceRange(start, end)));
