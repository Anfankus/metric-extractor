...
/**
 * Visitor to generate an intermediate tree (general, rooted, labeled, valued tree) out of a method
 * body.
 *
...
public class JavaMethodBodyConverter extends ASTVisitor {
   ...
   *
   * @param node the node to prepare for comment association
   */
  public void preVisit(ASTNode node) {
    ...
    while (i < fComments.size()) {
      ...
      if (previousNodeExistsAndIsNotTheFirstNode() && isCommentBetweenCurrentNodeAndLastNode(
          comment, node)) {
  ...
  /**
   * Associates a comment to code with the candidate triple {preceedingNode, comment,
   * succeedingNode}
   *
   * @param node succeeding node of the triple
   */
  ...
  /**
   * Calculates the proximity between the two given {@link ASTNode}. Usually one of the nodes is a
   * comment.
   *
   * @param nodeOne to calculate the proximity
   * @param nodeTwo to calculate the proximity
   * @return <code>2</code> if the comment node is on the same line as the other node,
   * <code>1</code> if they are on
  ...
  private int proximityRating(ASTNode left, ASTNode right) {
    ...
    if (sameLinePosition > -1) {
      betweenOneAndComment = betweenOneAndComment
          .substring(sameLinePosition + 1, betweenOneAndComment.length());
    }
   ...
   *
   * @param candidate to match with
   * @param comment to match for
  ...
  public boolean visit(BreakStatement breakStatement, BlockScope scope) {
    ...
    pushValuedNode(breakStatement,
        breakStatement.label != null ? String.valueOf(breakStatement.label) : "");
    return false;
  }
  ...
  public boolean visit(ForeachStatement foreachStatement, BlockScope scope) {
    ...
    pushValuedNode(foreachStatement,
        foreachStatement.elementVariable.printAsExpression(0, new StringBuffer())
   ...
   * @param scope in which the expression resides
   * @return <code>true</code> if the children of the expression should be visited,
   ...
   *
   * @param expression to end visit with
   * @param scope in which the visitor visits
   */
  ...
  public boolean visit(IfStatement ifStatement, BlockScope scope) {
    ...
    push(JavaEntityType.IF_STATEMENT, expression, ifStatement.sourceStart(),
  ...
  public boolean visit(CaseStatement caseStatement, BlockScope scope) {
    ...
    pushValuedNode(
        caseStatement,
        caseStatement.constantExpression != null ? caseStatement.constantExpression.toString()
            : "default");
    return false;
  }
  ...
  private void visitFinally(TryStatement node, BlockScope scope) {
    if (node.finallyBlock != null) {
      push(JavaEntityType.FINALLY, "", node.finallyBlock.sourceStart(),
          node.finallyBlock.sourceEnd());
  ...
  private void visitCatchClauses(TryStatement node, BlockScope scope) {
    if ((node.catchBlocks != null) && (node.catchBlocks.length > 0)) {
      ...
      for (int i = 0; i < node.catchArguments.length; i++) {
        int catchClauseSourceStart = retrieveStartingCatchPosition(start,
  ...
  private void push(EntityType label, String value, int start, int end) {
    ...
    n.setSourceCodeEntity(new SourceCodeEntity(value.trim(), label, new SourceRange(start, end)));
