...
/**
 * Implementation of the core tree differencing algorithm of Chawathe. This algorithm takes two
 * {@link Node} trees generates a matching between the nodes of both trees and calculates an edit
 * script of {@link TreeEditOperation} that transform the left into the right tree.
 * <p>
 * The implementation style is not clean, i.e., it uses single character method and variable names,
 * because the implementation should reflect the original algorithm from the paper as much as
 * possible.
 *
 * @author Beat Fluri
...
public class TreeDifferencer {
  ...
  /**
   * Calculates the edit script of {@link TreeEditOperation} between the left and the right {@link
   * Node} trees.
   *
   * @param left tree to calculate the edit script for
   * @param right tree to calculate the edit script for
   */
  ...
  private void editScript(Node left, Node right) {
    ...
    while (breathFirst.hasMoreElements()) {
      ...
      if (fRightToLeftMatchPrime.get(x) == null) {
        ...
        /* T1 */
        w = fRightToLeftMatchPrime.get(x);
        // Let v = p(w) in T1
  ...
  private boolean matchContains(Node v, Node y, Set<NodePair> match) {
    for (NodePair p : match) {
      if (((p.getLeft() == v) && (p.getRight() == y)) || ((p.getLeft() == y) && (p.getRight()
          == v))) {
        return true;
      }
    }
  ...
  private void extractLCS(int[][] b, List<Node> l, List<Node> r, int i, int j,
      HashSet<NodePair> lcs) {
    if ((i != 0) && (j != 0)) {
      if (b[i][j] == DIAG) {
