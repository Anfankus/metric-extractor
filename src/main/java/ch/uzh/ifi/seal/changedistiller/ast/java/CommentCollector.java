...
public class CommentCollector {
   ...
   *
   * @param compilationUnit from which the comments should be collected
   * @param source of the compilation unit
   */
  ...
  private Comment createComment(int[] positions) {
    ...
    if (end > 0) {
      ...
      if (start == 0) {
        if (fSource.charAt(1) == '/') {
          comment = new Comment(CommentType.LINE_COMMENT, start, end,
              fSource.substring(start, end));
        } else {
          comment = new Comment(CommentType.BLOCK_COMMENT, start, end,
              fSource.substring(start, end));
        }
