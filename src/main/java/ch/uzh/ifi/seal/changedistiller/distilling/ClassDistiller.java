...
public class ClassDistiller {
   ...
   *
   * @param classNode of which the changes should be extracted
   * @param classHistory to which the changes should be attached
   * @param leftASTHelper aids getting info from the left AST
   * @param rightASTHelper aids getting info from the right AST
   * @param refactoringProcessor to process potential refactorings
   * @param distillerFactory to create distillers
   */
   ...
   * @param refactoringProcessor to process potential refactorings
   * @param distillerFactory to create distillers
   * @param version the number or ID of the version associated to the changes being distilled
   */
  public ClassDistiller(
      StructureDiffNode classNode,
      ClassHistory classHistory,
  ...
  /**
   * Extract the {@link SourceCodeChange}s of the {@link StructureDiffNode} with which the class
   * distiller was initialized.
   */
  ...
  private void cleanupInnerClassHistories() {
    for (Iterator<ClassHistory> it = fClassHistory.getInnerClassHistories().values().iterator();
        it.hasNext(); ) {
  ...
  private void processClassDiffNode(StructureDiffNode diffNode) {
    ...
    if (fVersion != null) {
      ClassHistory classHistory = fClassHistory.createInnerClassHistory(
  ...
  private void processChanges(StructureDiffNode diffNode) {
    if (diffNode.isAddition()) {
      Insert insert =
          new Insert(fRootEntity, fRightASTHelper.createSourceCodeEntity(diffNode.getRight()),
              fParentEntity);
      fRefactoringContainer.addCandidate(new RefactoringCandidate(insert, diffNode));
    } else if (diffNode.isDeletion()) {
      Delete delete =
          new Delete(fRootEntity, fLeftASTHelper.createSourceCodeEntity(diffNode.getLeft()),
              fParentEntity);
  ...
  private StructureEntityVersion createInnerClassStructureEntity(StructureDiffNode diffNode) {
    if (fVersion != null) {
      return fRightASTHelper
          .createInnerClassInClassHistory(fClassHistory, diffNode.getRight(), fVersion);
    } else {
  ...
  private StructureEntityVersion createFieldStructureEntity(StructureDiffNode diffNode) {
    if (fVersion != null) {
      return fRightASTHelper
          .createFieldInClassHistory(fClassHistory, diffNode.getRight(), fVersion);
    } else {
  ...
  private StructureEntityVersion createMethodStructureEntity(StructureDiffNode diffNode) {
    if (fVersion != null) {
      return fRightASTHelper
          .createMethodInClassHistory(fClassHistory, diffNode.getRight(), fVersion);
    } else {
