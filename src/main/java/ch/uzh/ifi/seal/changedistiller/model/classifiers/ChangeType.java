 ...
 * <p>
 * Each type contains a {@link SignificanceLevel}, is marked whether it is a body change (e.g. found
 * inside a method body) or not and whether its significance level is unstable and may be lifted
 * (e.g. for a method renaming, the significance gets lifted if the method is public or protected).
 *
 * @author zubi
...
public enum ChangeType {
  ...
  private ChangeType(SignificanceLevel level, boolean isBodyChange,
      boolean hasUnstableSignificanceLevel) {
    fSignificance = level;
    fIsBodyChange = isBodyChange;
    fHasUnstableSignificanceLevel = hasUnstableSignificanceLevel;
  }

  public static String getAllChangeTypes() {
    StringBuffer result = new StringBuffer();
    for (int i = 0; i < ChangeType.values().length; i++) {
      result.append(ChangeType.values()[i].name() + " ");
    }
    return result.toString().trim();
  }

  public static int index(ChangeType source) {
    for (int i = 0; i < ChangeType.values().length; i++) {
      ChangeType target = ChangeType.values()[i];
      if (target == source) {
        return i;
      }
    }
  ...
  public static int index(String targetChangeType) {
    for (int i = 0; i < ChangeType.values().length; i++) {
      String sourceChangeType = ChangeType.values()[i].name().toLowerCase();
      if (sourceChangeType.equals(targetChangeType.toLowerCase())) {
        return i;
      }
    }
    return -1;
   ...
   *
   * @return <code>true</code> if this change is a body change, <code>false</code> if it is a
   * declaration change.
   */
   ...
   *
   * @return <code>true</code> if this change is a declaration change, <code>false</code> if it is a
   * body change.
   */
   ...
   *
   * @return <code>true</code> if the significance level is likely to be lifted, <code>false</code>
   * otherwise.
   */
  ...
  public static boolean isInMethodBody(ChangeType ct) {
    if (ct == ADDING_METHOD_OVERRIDABILITY || ct == ALTERNATIVE_PART_DELETE
        || ct == ALTERNATIVE_PART_INSERT
        || ct == COMMENT_DELETE || ct == COMMENT_INSERT || ct == COMMENT_MOVE
        || ct == COMMENT_UPDATE
        || ct == CONDITION_EXPRESSION_CHANGE || ct == DECREASING_ACCESSIBILITY_CHANGE
        || ct == DOC_DELETE
        || ct == DOC_INSERT || ct == DOC_UPDATE || ct == INCREASING_ACCESSIBILITY_CHANGE
        || ct == METHOD_RENAMING
        || ct == PARAMETER_DELETE || ct == PARAMETER_INSERT || ct == PARAMETER_ORDERING_CHANGE
        || ct == PARAMETER_RENAMING || ct == PARAMETER_TYPE_CHANGE
        || ct == REMOVING_METHOD_OVERRIDABILITY
        || ct == RETURN_TYPE_CHANGE || ct == RETURN_TYPE_DELETE || ct == RETURN_TYPE_INSERT
        || ct == STATEMENT_DELETE
        || ct == STATEMENT_INSERT || ct == STATEMENT_ORDERING_CHANGE
        || ct == STATEMENT_PARENT_CHANGE
        || ct == STATEMENT_UPDATE) {
      return true;
    }
    return false;
  ...
  public static void main(String[] arg) {
    for (ChangeType changeType : ChangeType.values()) {
      System.out.println(
          changeType.name().toLowerCase() + "," + changeType.fSignificance.name().toLowerCase());
    }

    int i1 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.DOC_DELETE);
    int i2 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.DOC_INSERT);
    int i3 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.DOC_UPDATE);
    int i4 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.COMMENT_DELETE);
    int i5 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.COMMENT_INSERT);
    int i6 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.COMMENT_MOVE);
    int i7 = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.COMMENT_UPDATE);
    Integer[] indices = {i1, i2, i3, i4, i5, i6, i7};
    List<Integer> filteredIndices = Arrays.asList(indices);

    int index = ch.uzh.ifi.seal.changedistiller.model.classifiers.ChangeType
        .index(ChangeType.ADDING_CLASS_DERIVABILITY);
    System.out.println(filteredIndices.contains(index));
  }
