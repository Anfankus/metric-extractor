...
public class CommentCleaner {
   ...
   *
   * @param source whos comments are visited
   */
   ...
   *
   * @param comment the comment that is added to the visited comment.
   */
  private void addToPreviousCommentBlock(Comment comment) {
    if (commentDoesNotContainNonNLS(comment)) {
      ...
      if (fVisitedComment != null) {
        ...
        fVisitedComment
            .setComment(
                fSource.substring(fVisitedComment.sourceStart(), fVisitedComment.sourceEnd()));
      } else {
  ...
  /**
   * Processes the given {@link Comment}. If the previous comment was a line comment and the current
   * is too, both are counted as "in the same block". {@link Comment.CommentType#JAVA_DOC JAVA_DOC}
  ...
  private void prepareConsecutiveComments(Comment comment) {
    ...
    if (previousCommentExistsAndIsLineComment() && comment.isLineComment()
        && commentDoesNotContainNonNLS(comment)) {
