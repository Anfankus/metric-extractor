...
/**
 * Processes {@link RefactoringCandidate}s from a {@link RefactoringCandidateContainer} and adds
 * resulting changes to the {@link ClassHistory}, and its member histories respectively.
 *
 * @author Beat Fluri
...
public class RefactoringCandidateProcessor {
  ...
  @Inject
  RefactoringCandidateProcessor(DistillerFactory distillerFactory,
      SourceCodeChangeClassifier changeClassifier) {
    fDistillerFactory = distillerFactory;
    fChangeClassifier = changeClassifier;
  }

  /**
   * Processes all {@link RefactoringCandidate}s in the {@link RefactoringCandidateContainer} and
   * stores all resulting changes to the {@link ClassHistory}.
   *
   * @param classHistory in which the refactorings took place
   * @param leftHelper to access the AST of the left side of the refactoring
   * @param rightHelper to access the AST of the right side of the refactoring
   ...
   * @param rightHelper to access the AST of the right side of the refactoring
   * @param candidates to process
   * @param version the number/ID of the version being distilled
   */
  public void processRefactoringCandidates(
      ClassHistory classHistory,
      ASTHelper<StructureNode> leftHelper,
  ...
  private void processFieldRefactoringCandidates(RefactoringCandidateContainer candidates) {
    checkRefactorings(candidates.getAddedFields(), candidates.getDeletedFields(),
        new FieldRefactoringHelper(
            fClassHistory,
            fRightASTHelper));
  }

  private void processMethodRefactoringCandidates(RefactoringCandidateContainer candidates) {
    checkRefactorings(candidates.getAddedMethods(), candidates.getDeletedMethods(),
        new MethodRefactoringHelper(
            fClassHistory,
            fRightASTHelper));
  }
  ...
  private void processRefactorings(
    ...
    for (RefactoringPair pair : refactorings) {
      ...
      if (fVersion != null) {
        structureEntityVersion = refactoringHelper
            .createStructureEntityVersionWithID(rightDrn, fVersion);
      } else {
      ...
      if (isRenaming(leftDrn, rightDrn, refactoringHelper)) {
        if (fVersion != null) {
          structureEntityVersion =
              refactoringHelper
                  .createStructureEntityVersionWithID(leftDrn, rightDrn.getFullyQualifiedName(),
                      fVersion);
        } else {
          structureEntityVersion =
              refactoringHelper
                  .createStructureEntityVersion(leftDrn, rightDrn.getFullyQualifiedName());
        }
        Update upd =
            new Update(
                structureEntityVersion,
                ...
                leftRoot.getSourceCodeEntity());
        structureEntityVersion
            .addAllSourceCodeChanges(fChangeClassifier.classifySourceCodeChanges(Arrays
                .asList(upd)));
      }
  ...
  private void processRemainingDiffs(StructureEntityVersion clazz,
    ...
    for (RefactoringCandidate candidate : candidates) {
      if (!candidate.isRefactoring()) {
        List<SourceCodeChange> classifiedChanges =
            fChangeClassifier
                .classifySourceCodeChanges(Arrays.asList(candidate.getSourceCodeChange()));
