...
/**
 * Converts {@link TreeEditOperation}s to {@link SourceCodeChange}s and attaches them to a {@link
 * StructureEntityVersion} in which the changes happened.
 *
 * @author Beat Fluri
 */
public final class SourceCodeChangeConverter {
   ...
   *
   * @param structureEntity in which the {@link SourceCodeChange}s happened
   */
  ...
  /**
   * Converts a {@link TreeEditOperation}s to a {@link SourceCodeChange}s and adds it to the {@link
   * StructureEntityVersion}.
   * <p>
   * A {@link SourceCodeChange} is added if the conversion didn't result in a <code>null</code>
   * value.
   *
   * @param operation to convert
   */
  public void addTreeEditOperationAsSourceCodeChange(TreeEditOperation operation) {
    SourceCodeChange scc = null;
    switch (operation.getOperationType()) {
      case INSERT:
        scc = fSourceCodeChangeFactory
            .createInsertOperation(fStructureEntity, (InsertOperation) operation);
        break;
      case DELETE:
        scc = fSourceCodeChangeFactory
            .createDeleteOperation(fStructureEntity, (DeleteOperation) operation);
        break;
      case MOVE:
        scc = fSourceCodeChangeFactory
            .createMoveOperation(fStructureEntity, (MoveOperation) operation);
        break;
      case UPDATE:
        scc = fSourceCodeChangeFactory
            .createUpdateOperation(fStructureEntity, (UpdateOperation) operation);
        break;
      default:
  ...
  /**
   * Converts a {@link List} of {@link TreeEditOperation}s into a {@link List} of {@link
   * SourceCodeChange}s and adds them to the {@link StructureEntityVersion}.
   * <p>
   * A {@link SourceCodeChange} is added if the conversion didn't result in a <code>null</code>
   * value.
   *
   * @param operations to convert
   */
